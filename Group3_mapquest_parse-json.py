...
def checkInput():
    ...
    if(entered_From == '' or entered_To == ''):
        ...
        getMapQuest(entered_From, entered_To,
                    entered_RouteType, entered_Avoid, language)
...
def displayInfo(orig, dest, routeType, avoid, json_data, json_status, url):
    Label(right_frame, text="Directions from " + (orig) + " to " + (dest),
          bg='white', justify="left").grid(row=1, column=0, padx=5, pady=5, sticky='w')

    Label(right_frame, text="Route Type: " + (routeType), bg='white',
          justify="left").grid(row=2, column=0, padx=5, pady=2, sticky='w')

    Label(right_frame, text="Trip Duration: " + (json_data["route"]["formattedTime"]),
          bg='white', justify="left").grid(row=3, column=0, padx=5, pady=2, sticky='w')

    Label(right_frame, text="Avoid: " + (avoid), bg='white',
          justify="left").grid(row=4, column=0, padx=5, pady=2, sticky='w')

    Label(right_frame, text="Distance: ", bg='white', justify="left").grid(
        row=5, column=0, padx=5, pady=0, sticky='w')
    Label(right_frame, text="\tIn Miles: " + str(json_data["route"]["distance"]),
          bg='white', justify="left").grid(row=6, column=0, padx=0, pady=0, sticky='w')
    Label(right_frame, text="\tIn Kilometers: " + str("{:.2f}".format(
        (json_data["route"]["distance"])*1.61)), bg='white', justify="left").grid(row=7, column=0, padx=0, pady=0, sticky='w')

    routeFrame = Frame(right_frame, width=1000, height=400, bg='white')
    routeFrame.grid(row=8, column=0, padx=5, pady=20)

    counter = 1
    Label(routeFrame, text="Distance", bg='white', justify="left").grid(
        row=0, column=0, padx=0, pady=0, sticky='w')
    Label(routeFrame, text="  Direction", bg='white', justify="left").grid(
        row=0, column=1, padx=0, pady=0, sticky='w')
    for each in json_data["route"]["legs"][0]["maneuvers"]:

        if (counter <= 25):
            Label(routeFrame, text=" (" + str("{:.2f}".format((each["distance"])*1.61) + " km)"),
                  bg='white', justify="left").grid(row=counter, column=0, padx=0, pady=0, sticky='w')
            Label(routeFrame, text='  '+(each["narrative"]), bg='white', justify="left").grid(
                row=counter, column=1, padx=0, pady=0, sticky='w')
        counter += 1
...
Label(left_frame, text="Map Quest", bg='white').grid(
    row=0, column=0, padx=5, pady=5)
Label(left_frame, text="MapQuest provides simple directions \n and creates suitable route points \n depending on your preferred route type!",
      bg='white').grid(row=1, column=0, padx=5, pady=5)

Label(left_frame, text="Please enter the following details",
      bg='white').grid(row=2, column=0, padx=10, pady=5)
...
Label(inputRouteType, text='Route Type', bg='white').grid(
...
Label(inputLocale, text='Route Language', bg='white').grid(
    row=0, column=0, sticky='w')
